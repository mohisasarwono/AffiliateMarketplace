...
public interface StoreRepositories extends JpaRepository<Store, Long> {
    ...
    @Query(value="select i.id as iId from mst_store as s "
            + "join mst_item as i on s.id = i.store_id where s.name ilike(concat('%',:name,'%')) and i.status = 1 and s.status = 1 and i.expired_date >= now() order by i.id ", nativeQuery = true)
    List<Long> getItemByNStoreName(@Param("name")String name);
    
    @Query(value="select i.id as iId from mst_store as s "
            + "join mst_item as i on s.id = i.store_id where i.name ilike(concat('%',:name,'%')) and i.status = 1 and i.expired_date >= now() order by i.id ", nativeQuery = true)
    List<Long> getItemByItemName(@Param("name")String name);
    
    @Query(value="select i.id as iId from mst_item as i "
            + "where i.type ilike(:type) and i.status = 1 and i.expired_date >= now() order by i.id", nativeQuery = true )
    ...
    "where c.total_transaction >= :totalTranscation and s.status = 1 and i.expired_date >= now() order by i.id ", nativeQuery = true)
    List<Long> getItemByTotalTransaction(@Param("totalTranscation")Integer totalTransaction);
    
    @Query(value="select id from mst_item where commission_price_or_percentage>=:commissionRange and commission_status =:commissionStatus and status= 1 and expired_date >= now() order by id ", nativeQuery = true)
