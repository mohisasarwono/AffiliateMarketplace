...
public interface CommissionRepositories extends JpaRepository<Commission, Long> {
    ...
    @Query(value="select s.id as storeId, s.name as storeName, sum(p.click_counter) as clickCounter, sum(c.total_commission_amount) as totCommAm, sum(c.total_transaction) as totTransac, p.peripheral_link as pLink, sum(cd.recurring_counter) as cDRec, i.recurring as iRec from mst_peripheral as p " +
        ...
        "group by s.id, pLink, i.id ", nativeQuery = true)
    List<Object[]>getDataForSummary(@Param("promoterId")Long promoterId);
    
    @Query(value="select s.id as storeId, s.name as storeName, sum(p.click_counter) as clickCounter, sum(cd.commission_amount) as totCommAm, count(cd.id) as totTransac, p.peripheral_link as pLink,\n" +
"sum(cd.recurring_counter) as cDRec, i.recurring as iRec " +
