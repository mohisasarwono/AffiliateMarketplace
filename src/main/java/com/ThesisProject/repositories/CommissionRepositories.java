...
public interface CommissionRepositories extends JpaRepository<Commission, Long> {
    ...
    @Query(value="select s.id as storeId, s.name as storeName, sum(p.click_counter) as clickCounter, sum(cd.commission_amount) as totCommAm, count(cd.id) as totTransac from mst_peripheral as p " +
        "join mst_item as i on p.item_id = i.id " +
        "join mst_store as s on s.id = i.store_id " +
        "join trx_commission as c on c.peripheral_id = p.id " +
        ...
        "join mst_referral_code as r on r.id = p.referral_id " +
        "join mst_promoter as pr on pr.id = r.promoter_id " +
        "where pr.id = :promoterId and cd.transaction_date between TO_DATE(:startDate,'YYYY-MM-DD HH:MM:SS') and TO_DATE(:endDate,'YYYY-MM-DD HH:MM:SS')  " +
        "group by s.id", nativeQuery = true)
...
"        group by s.id, pr.id", nativeQuery = true)
    ...
    @Query(value="select s.id as storeId, pr.name as promoterName, sum(p.click_counter) as clickCounter, sum(cd.commission_amount) as totCommAm, count(cd.id) as totTransac from mst_peripheral as p \n" +
"        join mst_item as i on p.item_id = i.id " +
"        join mst_store as s on s.id = i.store_id " +
"        join trx_commission as c on c.peripheral_id = p.id " +
"        join trx_commission_detail as cd on cd.commission_id = c.id "+
